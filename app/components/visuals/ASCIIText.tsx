...
class CanvAscii {
  ...
  constructor(
    { text, asciiFontSize, textFontSize, textColor, planeBaseHeight, enableWaves }: AsciiConfig,
  ...
  setMesh() {
    ...
    const planeWidth = baseHeight * textAspect;
  ...
  clear() {
    this.scene.traverse((obj) => {
      if (obj instanceof THREE.Mesh && typeof obj.material === "object" && obj.material !== null) {
        Object.keys(obj.material).forEach((key) => {
          const matProp = obj.material[key as keyof typeof obj.material];
          if (matProp && typeof matProp === "object" && "dispose" in matProp) {
            const maybeDisposable = matProp as { dispose?: () => void };
        ...
        observer = new IntersectionObserver(
          async ([entry]) => {
            ...
            if (entry.isIntersecting && entry.boundingClientRect.width > 0 && entry.boundingClientRect.height > 0) {
              const { width: entryWidth, height: entryHeight } = entry.boundingClientRect;
              observer?.disconnect();
              observer = null;
