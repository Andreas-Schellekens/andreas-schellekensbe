...
const ScrollStack = ({
  ...
  const getElementOffset = useCallback(
    (element) => {
      ...
      if (!useWindowScroll) {
        ...
        if (scroller) {
          let scrollerTop = 0;
          let scrollerNode = scroller;
          while (scrollerNode) {
            scrollerTop += scrollerNode.offsetTop || 0;
            scrollerNode = scrollerNode.offsetParent;
          }
  ...
  const captureStaticOffsets = useCallback(() => {
  ...
  useLayoutEffect(() => {
    ...
    return () => {
      ...
      imageListeners.forEach(({ image, onLoad }) => {
  ...
  const windowClass = useWindowScroll ? "scroll-stack-window" : "";

  return (
    <div className={`scroll-stack-scroller ${windowClass} ${className}`.trim()} ref={scrollerRef}>
